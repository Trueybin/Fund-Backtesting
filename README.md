...
本项目是一个本地运行的场外基金定投测算 MVP。它使用 AkShare 获取开放式基金历史单位净值，按固定金额、日/周/月频率模拟定投，并展示收益和每日资产曲线。
...
- 买入份额 = 净申购金额 / 当日单位净值
- 累计投入按每笔定投金额（含申购费）累计
- 每日定投按可买入交易日执行，不按自然日补买周末、节假日或调休工作日
- 每周、每月计划遇到非交易日时，可选择顺延至下一个交易日或跳过；顺延不会超出回测结束日
- 若结束日不是有净值的交易日，最终估值使用结束日前最近一个可用净值日

历史净值会写入 `backend/data/fund_cache.db`，同一基金的可覆盖请求会在 24 小时内直接使用本地缓存。
